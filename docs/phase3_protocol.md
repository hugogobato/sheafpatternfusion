...
**Status (2026-08-26, REVISED SAME DAY): fleet of 18 self-contained CPU notebooks generated (v2). Revision fixes after the first Colab wave: (1) `pooled_map_deadline` now keys jobs by `iid` OR `instance_id` and only cancels queued futures after a deadline trip -- the earlier cancel-pass ran every iteration and crashed cycattack/signal shards with `KeyError: 'iid'`; (2) NHANES registry entries replaced by verified OpenML sets (colic, cylinder-bands, arrhythmia) and the two wrong UCI slugs corrected (automobile.zip, soybean+large.zip); (3) a non-tabular payload guard + missing-rate histogram diagnostics added so transient UCI block pages report as LOAD FAILED instead of silently zero-pooling; (4) a `ds_reports.values()`->`items()` aggregation bug fixed; (5) `SIGNAL_FEATURES` had a broken `\n` escape and the downstream correlation now falls back to the recomputed share-pinned width for rows lacking a stored one (previously n=0). All 18 notebooks regenerated; every family re-smoked locally at toy budgets. Re-upload ALL 18 notebooks; the old versions must not be reused (checkpoint JSONLs resume cleanly, so partial results from crashed runs are retained).
...
|---|---|---|---|---|---|
| `nb30_a_prevalence.ipynb` | WP3.0a natural-prevalence scan | 1 | self-contained, downloads 11 public datasets (6 UCI zips + 5 OpenML sets, EVERY entry verified loadable 2026-08-26; NHANES replaced: CDC soft-404s its XPT paths), row-cap 30k, pool<=24 vars, subset sizes 3-6 up to 60k/subset, 30k/dataset, min_patterns>=4, min_support 1 vs 5 robustness, missing-window sensitivity, column-permutation negative control, adaptive row-level bootstrap (target ~55 min collective) | ~1-1.5 h | `results/phase3/prevalence_scan.{csv,json}` |
